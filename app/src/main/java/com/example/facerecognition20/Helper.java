...
import com.example.facerecognition20.mtcnn.Align;
import com.example.facerecognition20.mtcnn.Box;
...
public class Helper {
    ...
    public static Bitmap detect_and_crop_face(Context context, MTCNN mtcnn, Bitmap image) {
        ...
        if (boxes.isEmpty()) {
            ...
            // Detect faces again after correction
            boxes = mtcnn.detectFaces(image, image.getWidth() / 5);

            // Check if the list is still not empty
            if (boxes.isEmpty()) {
                Toast.makeText(context, "No face detected after correction", Toast.LENGTH_LONG).show();
                return null;
            }

            // Get the first box after correction
            box = boxes.get(0);
